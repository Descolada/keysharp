...
| Image | 🟢 Full | 🟡 Partial | ⚪ Unknown | ⚪ Unknown | Cross-platform image class: capture (desktop/monitor/rect/window), load (file/bitmap), lazy transforms (scale/rotate/flip/crop), output (save/bitmap), pixel access and search. Replaces the earlier ImageCapture() function. |
...
| WinEvent.Close() | 🟢 Full | 🟡 Partial | 🟡 Partial | 🟠 Planned | Subscribes to window destruction, or (for a DetectHiddenWindows-off subscription) hiding/cloaking. Tracks the set of matching top-level windows while they are alive (mirroring the reference library) so it fires reliably even though the window is gone by the time the event arrives. |
| WinEvent.Count | 🟢 Full | 🟢 Full | 🟢 Full | 🟢 Full | Remaining number of times the callback will fire (-1 = unlimited). |
| WinEvent.Create() | 🟢 Full | 🟡 Partial | 🟡 Partial | 🟠 Planned | Subscribes to top-level window creation. macOS requires AXObserver wiring (not yet implemented). |
...
| WinEvent.Show() | 🟢 Full | 🟡 Partial | 🟡 Partial | 🟠 Planned | Subscribes to a window becoming visible (mapped/shown). |
