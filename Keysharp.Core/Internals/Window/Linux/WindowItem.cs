	...
	internal class WindowItem : WindowItemBase//Do we want to prefix each of these derived classes with Windws/Linux?//TODO
	{
		...
		internal override bool Bottom
		{
			set
			{
				...
				if (value)
				{
					//Ks.OutputDebugLine($"Bottom: about to call XLowerWindow().");
					_ = Xlib.XLowerWindow(xwindow.XDisplay.Handle, xwindow.ID);
				}
				else
				{
					//Ks.OutputDebugLine($"Bottom: about to call XRaiseWindow().");
		...
		internal override FormWindowState WindowState
		{
			...
			set
			{
				...
				{
					//Ks.OutputDebugLine($"Window {current_state} == {value}, so doing nothing.");
					return;
				}
					...
					case FormWindowState.Normal:
					{
						lock (WindowManager.xLibLock)
						{
							if (current_state == FormWindowState.Minimized)
							{
								...
								//Ks.OutputDebugLine($"Window was minimized, so setting to normal.");
							...
							{
								//Ks.OutputDebugLine($"Window was maximized, so setting to normal.");
							...
							//else
							//  Ks.OutputDebugLine($"Window was {current_state}, so doing nothing.");
						}
		...
		internal override Keysharp.Internals.Window.POINT ClientToScreen()
		{
			if (IsSpecified)
			{
				int x = 0, y = 0;
