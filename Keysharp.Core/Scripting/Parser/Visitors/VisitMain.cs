            ...
            if (parser.IsVarDeclaredGlobally(text) == null)
            {
                switch (text.ToLowerInvariant())
                {
                    ...
                    case "a_scriptfullpath":
                        parser.mainClass.Body.Add(CreatePublicConstant("A_ScriptFullPath", typeof(Primitive), File.Exists(parser.fileName) ? Path.GetFullPath(parser.fileName) : parser.fileName));
                        break;
                }
            }
