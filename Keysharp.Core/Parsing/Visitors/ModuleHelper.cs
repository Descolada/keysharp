...
namespace Keysharp.Parsing
{
	internal partial class VisitMain
	{
		...
		private void AddModuleAliasImport(Parser.Module module, Parser.Module targetModule, string alias, bool exportMember)
		{
			...
			if (targetModule.DefaultExport == Parser.Module.DefaultExportKind.None)
			{
				if (ReferenceEquals(module, targetModule))
				{
					var prop = CreateImportProperty(aliasName, targetAccess, includeSetter: false);
					if (exportMember)
					{
		...
		private static bool IsAhkModuleType(Type type) =>
			type != null && typeof(Keysharp.Runtime.Ahk).IsAssignableFrom(type);
		...
		private bool TryResolveAhkNamedImport(string importName, out ExportKind kind, out ExpressionSyntax targetAccess, out bool includeSetter)
		{
			...
			if (rd.flatPublicStaticMethods.TryGetValue(importName, out var mi))
			{
				...
				targetAccess = SyntaxFactory.InvocationExpression(
					CreateMemberAccess("Keysharp.Builtins.Functions", "Func")
				).WithArgumentList(
		...
		private static IEnumerable<ExternalMember> EnumerateExternalMembers(Type type)
		{
			...
			Keysharp.Internals.Invoke.Reflections.FindAndCacheStaticMethod(type, "", -1);
			Keysharp.Internals.Invoke.Reflections.FindAndCacheProperty(type, "", -1);
			Keysharp.Internals.Invoke.Reflections.FindAndCacheField(type, "");
