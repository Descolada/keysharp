...
namespace Keysharp.Core.Linux
{
	...
	internal class LinuxHookThread : Keysharp.Core.Common.Threading.HookThread
	{
		...
		private DateTime lastSendTimestampUtc;
		...
		private char[] lastChars = System.Array.Empty<char>();
		private int lastCharCount = 0;

		private void CaptureCharsFromEvent(KeyboardHookEventArgs e)
		{
			lastCharCount = 0;
			if (!IsX11Available)
				return;

			int keycode = e.RawEvent.Keyboard.RawCode;
			if (keycode == 0)
				return;

			var mods = CurrentModifiersLR();
			bool shift = (mods & (MOD_LSHIFT | MOD_RSHIFT)) != 0;
			bool altgr = (mods & MOD_RALT) != 0 || (mods & MOD_LALT) != 0 && (mods & MOD_LCONTROL) != 0; // rough AltGr detection

			// XKeycodeToKeysym index: 0=normal,1=shift,2=mode_switch(AltGr),3=mode_switch+shift
			int idx = (altgr ? 2 : 0) + (shift ? 1 : 0);
			uint ks = (uint)XDisplay.Default.XKeycodeToKeysym(keycode, idx);
			if (ks == 0)
				return;

			if (KeysymToChar(ks, out var ch))
			{
				if (lastChars.Length < 1)
					lastChars = new char[1];
				lastChars[0] = ch;
				lastCharCount = 1;
			}
		}

		private static bool KeysymToChar(uint ks, out char ch)
		{
			ch = '\0';
			...
			switch (ks)
			{
				case 0xFF08: ch = '\b'; return true; // BackSpace
				case 0xFF09: ch = '\t'; return true; // Tab
				case 0xFF0D: ch = '\r'; return true; // Return
				case 0xFF0A: ch = '\n'; return true; // Linefeed
				case 0xFF80: ch = ' '; return true; // KP_Space
				case 0x20: ch = ' '; return true;   // Space
			}
		...
		*/
		...
		private void ForceReleaseKeyX11(uint vk, bool markHotstring)
		{
			...
			uint xKeycode = XDisplay.Default.XKeysymToKeycode((IntPtr)ks);
