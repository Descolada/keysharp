...
using ExportKind = Keysharp.Parsing.Parser.Module.ExportKind;
using Keysharp.Builtins;
using Microsoft.CodeAnalysis;
using Microsoft.CodeAnalysis.CSharp;
using Microsoft.CodeAnalysis.CSharp.Syntax;
using Microsoft.CodeAnalysis.Text;
using ParserSymbolInfo = Keysharp.Parsing.Parser.SymbolInfo;
    ...
    internal partial class VisitMain : MainParserBaseVisitor<SyntaxNode>
    {
		...
		public override SyntaxNode VisitProgram([NotNull] ProgramContext context)
        {
			if (parser.isFirstModulePass || parser.compilationUnit == null)
			{
				...
				parser.AddAssembly("Keysharp.Runtime.AssemblyBuildVersionAttribute", Accessors.A_AhkVersion);
				...
				var mainScriptVarDeclaration = Parser.CreateFieldDeclaration(
					modifierList,
					CreateQualifiedName("Keysharp.Runtime.Script"),
					MainScriptVariableName,
					SyntaxFactory.ObjectCreationExpression(
						CreateQualifiedName("Keysharp.Runtime.Script"),    // the type to construct
			...
			if (parser.persistent)
			{
				programAutoExecFunc.Body.Add(
					SyntaxFactory.ExpressionStatement(
						SyntaxFactory.InvocationExpression(
							CreateMemberAccess("Keysharp.Builtins.Flow", "Persistent")
						)
					)
				);
			}
			programAutoExecFunc.Body.Add(
				SyntaxFactory.ExpressionStatement(
					SyntaxFactory.InvocationExpression(
						CreateMemberAccess("Keysharp.Runtime.Keyboard.HotkeyDefinition", "ManifestAllHotkeysHotstringsHooks")
					)
				)
			);
		...
		)
		{
			...
			static NameSyntax Q(string dotted) => Keysharp.Parsing.Parser.CreateQualifiedName(dotted); // VisitorHelper
			...
			var aScriptName = SMA("Keysharp.Builtins.Accessors", "A_ScriptName");                         // Accessors.A_ScriptName
			...
			var singleInstanceGate = SyntaxFactory.IfStatement(
				Call(
					SMA("Keysharp.Runtime.Script", "HandleSingleInstance"),
					aScriptName,
					SyntaxFactory.MemberAccessExpression(
						SyntaxKind.SimpleMemberAccessExpression,
						Q("Keysharp.Runtime.eScriptInstance"),
			...
			tryStatements.Add(
				SyntaxFactory.ExpressionStatement(
					Call(SMA("Keysharp.Builtins.Env", "HandleCommandLineParams"), SyntaxFactory.IdentifierName("args"))
				)
			);
			...
			var handleKeysharpException = SyntaxFactory.ExpressionStatement(
				Call(
					SMA("Keysharp.Runtime.Script", "TryProcessKeysharpException"),
					mainScriptId,
					...
					.WithVariables(
						SyntaxFactory.SingletonSeparatedList(
							SyntaxFactory.VariableDeclarator(SyntaxFactory.Identifier("ex"))
								.WithInitializer(
									SyntaxFactory.EqualsValueClause(
										Call(SMA("Keysharp.Runtime.Flow", "UnwrapException"), SyntaxFactory.IdentifierName(mainexId))
									)
								)
						)
					)
			...
			// if (ex is Keysharp.Builtins.Flow.UserRequestedExitException) return System.Environment.ExitCode;
			var isUserRequestedExitPattern = SyntaxFactory.BinaryExpression(
				SyntaxKind.IsExpression,
				SyntaxFactory.IdentifierName("ex"),
				Q("Keysharp.Builtins.Flow.UserRequestedExitException")
			);
			...
			// if (ex is Keysharp.Builtins.KeysharpException kserr) { ... } else { ... }
			var isKsErrorPattern = SyntaxFactory.IsPatternExpression(
				SyntaxFactory.IdentifierName("ex"),
				SyntaxFactory.DeclarationPattern(
					Q("Keysharp.Builtins.KeysharpException"),
			...
			var handleUncaughtGeneric = SyntaxFactory.ExpressionStatement(
				Call(
					SMA("Keysharp.Runtime.Script", "TryProcessUnhandledException"),
					mainScriptId,
					SyntaxFactory.IdentifierName("ex")
				)
			);
			...
			var safeExit = SyntaxFactory.ExpressionStatement(
				Call(SMA("Keysharp.Runtime.Script", "SafeExit"), N(1))
			);
        ...
        public override SyntaxNode VisitPropertyExpressionAssignment([NotNull] PropertyExpressionAssignmentContext context)
        {
            ...
            // Keysharp.Runtime.Script.Vars[expression] should extract expression
        ...
        public override SyntaxNode VisitIfStatement([NotNull] IfStatementContext context)
        {
			...
			var ifStatement = SyntaxFactory.IfStatement(
                SyntaxFactory.InvocationExpression(
                    CreateMemberAccess("Keysharp.Runtime.Script", "IfTest"),
                    argumentList),
                ifBlock
            );
        ...
        public override SyntaxNode VisitThrowStatement([NotNull] ThrowStatementContext context)
        {
            if (context.singleExpression() == null)
                return SyntaxFactory.ThrowStatement(
                    ...
                    SyntaxFactory.ObjectCreationExpression(
                        CreateQualifiedName("Keysharp.Builtins.Error"),
            ...
            {
                // Wrap the literal in Keysharp.Builtins.Error
                return SyntaxFactory.ThrowStatement(
                    SyntaxFactory.Token(SyntaxKind.ThrowKeyword),
                    SyntaxFactory.ObjectCreationExpression(
						CreateQualifiedName("Keysharp.Builtins.Error"),
            ...
            return SyntaxFactory.ThrowStatement(
				SyntaxFactory.Token(SyntaxKind.ThrowKeyword),
                SyntaxFactory.CastExpression(CreateQualifiedName("Keysharp.Builtins.Error"), expression),
                PredefinedKeywords.SemicolonToken
            );
        }
        ...
        {
            // Wrap the condition in Keysharp.Runtime.Script.IfTest(condition) to force a boolean
                        ...
                        SyntaxFactory.SingletonSeparatedList(
                            ...
                            .WithInitializer(
                                SyntaxFactory.EqualsValueClause(
									PredefinedKeywords.EqualsToken,
									SyntaxFactory.ObjectCreationExpression(
										CreateQualifiedName("Keysharp.Builtins.Array")
									)
                                    .WithArgumentList(
        ...
        public override SyntaxNode VisitArrayLiteral(ArrayLiteralContext context)
        {
            ...
            // Wrap the array initializer in a call to 'new Keysharp.Builtins.Array(...)'
            var keysharpArrayCreation = SyntaxFactory.ObjectCreationExpression(
                CreateQualifiedName("Keysharp.Builtins.Array"), // Class name: Keysharp.Builtins.Array
                elementsArgList,
        ...
        public override SyntaxNode VisitMapLiteral([NotNull] MapLiteralContext context)
        {
			...
			return SyntaxFactory.ObjectCreationExpression(
				CreateQualifiedName("Keysharp.Builtins.Map"), // Class name: Keysharp.Builtins.Map
				argumentList,
				null // No object initializers
			);
        }
