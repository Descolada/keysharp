...
namespace Keysharp.Tests
{
	public class BuiltInVarTests : TestRunner
	{
		...
		public void PropsDateTime()
		{
			...
			Assert.AreEqual(now.Year, (int)yyyy);
			Assert.AreEqual(now.ToString("MM"), (string)mm);
			Assert.AreEqual(now.ToString("dd"), (string)dd);
			Assert.AreEqual(now.ToString("MMMM"), (string)mmmm);
			Assert.AreEqual(now.ToString("MMM"), (string)mmm);
			Assert.AreEqual(now.ToString("dddd"), (string)dddd);
			Assert.AreEqual(now.ToString("ddd"), (string)ddd);
			Assert.AreEqual((long)now.DayOfWeek + 1L, (long)wday);
			Assert.AreEqual(now.DayOfYear, (int)yday);
			Assert.AreEqual($"{now:yyyy}{week:D2}", (string)yweek);
			Assert.AreEqual(now.ToString("HH"), (string)hh);
			Assert.AreEqual(now.ToString("mm"), (string)min);
			Assert.AreEqual(now.ToString("ss"), (string)ss);
			...
			Assert.AreEqual((string)full, full2);
