					...
					callFunc = (inst, obj) =>
					{
						...
						else
						{
							...
							for (; oi < objLength && pi < ParamLength; pi++)
							{
								if (pi == variadicParamIndex)
								{
									...
									{
										var om1 = obj[pi] == null ? 0 : (objLength - variadicParamIndex) - stopVarIndexDistanceFromEnd;//obj[pi] == null means null was passed for the variadic parameter.
						...
						if (ParamLength > 1)
						{
							...
							if (variadicParamIndex != -1)
							{
								...
								{
									if (pi == variadicParamIndex)//Expand the variadic args back into the flat array.
									{
