...
{
	/// <summary>
	/// Win32 read/write ops for a single window, keyed by handle. NOT a WindowInfo (the one neutral
	/// <see cref="Keysharp.Internals.Window.WindowInfo"/> is what callers see) — it is the internal helper that
	/// <see cref="WindowsWindow"/> drives. (TODO: inline these reads directly into WindowsWindow and slim this to
	/// the IWin32Window owner adapter + GetMenuItemId/SetForegroundWindowEx utilities — a Windows-machine task.)
	/// </summary>
	internal sealed class Win32Window : IWin32Window
	{
		...
		internal bool IsIconic => WindowState == FormWindowState.Minimized;

		internal Win32Window(nint handle) => Handle = handle;
		...
		public bool Active
		{
			get
			{
				if (IsSpecified && WindowQuery.ActiveWindow is WindowInfoBase item)
				{
					...
					//Keysharp.Builtins.File.FileAppend($"item.Handle: {item.Handle.ToInt64()}, item.Title: {item.Title}, Handle: {Handle.ToInt64()}, Title: {Title}\n", "out.txt");
		...
		public IEnumerable<nint> ChildHandles
		{
			get
			{
				var children = new HashSet<nint>(lastChildCount);
				...
				if (Control.FromHandle(Handle) is Form form)
				{
					form.Invoke(() =>
					{
						foreach (var ctrl in form.GetAllControlsRecursive<Control>())
							_ = children.Add(ctrl.Handle);//HashSet takes care of avoiding dupes.
					});
				}
		...
		public string ClassName => className ??= IsSpecified ? WindowsAPI.GetClassName(Handle) : string.Empty;
		...
		public bool Exists => IsSpecified&& WindowsAPI.IsWindow(Handle);
		...
		public bool IsHung => Handle == 0 ? false : WindowsAPI.IsHungAppWindow(Handle);
		...
		public nint TopLevelHandle => WindowsAPI.GetNonChildParent(Handle);

		public nint ParentHandle => WindowsAPI.GetAncestor(Handle, gaFlags.GA_PARENT);
		...
		public bool Visible
		{
			get => IsSpecified && (ParentHandle != 0 ? WindowsAPI.IsWindowVisible(Handle) : (WindowsAPI.IsWindowVisible(Handle) && !WindowsAPI.IsWindowCloaked(Handle)));
		...
		/// <returns></returns>
		internal static nint SetForegroundWindowEx(Win32Window win, bool backgroundActivation = false)
		{
			if (win == null)
				return 0;
			...
			{
				var foregroundwin = WindowQuery.CreateWindow(origForegroundWnd);
		...
		public bool Close() => IsSpecified&& WindowsAPI.PostMessage(Handle, WindowsAPI.WM_CLOSE, 0, 0);
		...
		public bool Hide() => IsSpecified&& WindowsAPI.ShowWindow(Handle, WindowsAPI.SW_HIDE);
		...
		//public WindowInfoBase RealChildWindowFromPoint(Point location)
		//{
		//  WindowInfoBase child = null;

		//  if (IsSpecified)
		//      child = new WindowInfo(WindowsAPI.RealChildWindowFromPoint(Handle, location));

		//  return child;
		//}

		public bool Redraw() => IsSpecified&& WindowsAPI.InvalidateRect(Handle, 0, true);
		...
		private static nint AttemptSetForeground(nint targetWindow, nint foreWindow)
		{
			...
			Keysharp.Internals.Flow.SleepWithoutInterruption(10);
