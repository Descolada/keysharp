...
namespace Keysharp.Core.Linux
{
	...
	internal class LinuxHookThread : Keysharp.Core.Common.Threading.HookThread
	{
		...
		private void OnKeyPressed(object? sender, KeyboardHookEventArgs e)
		{
			...
			if (e.IsEventSimulated) { Console.WriteLine($"[Hook] KeyDown vk={vk} filtered simulated"); return; }
			if (InjectedActive()) { Console.WriteLine($"[Hook] KeyDown vk={vk} filtered injected"); return; }
			if (sendInProgress) { Console.WriteLine($"[Hook] KeyDown vk={vk} filtered sendInProgress"); return; }
				...
				if (hsArmed && hsArmedEnds.Count > 0)
				{
					...
					if (chosen.HasValue)
					{
						...
						if (hs != null)
						{
							...
							char? evt = ' ';
							if (KeyboardMouseSender.HotInputLevelAllowsFiring(hs.inputLevel, 0, ref evt))
							{
								// Case conform (same as above)
								var caseMode = CaseConformModes.None;
								if (hs.conformToCase)
								{
									var span = System.Runtime.InteropServices.CollectionsMarshal.AsSpan(hm.hsBuf);
									...
									if (cap > 0)
									{
										if (up == cap) caseMode = CaseConformModes.AllCaps;
										else if (up == 0) caseMode = CaseConformModes.None;
										else if (char.IsLetter(span[0]) && char.IsUpper(span[0])) caseMode = CaseConformModes.FirstCap;
									}
								}
		...
		private void OnKeyReleased(object? sender, KeyboardHookEventArgs e)
		{
			...
			if (e.IsEventSimulated)
			{
				...
				Console.WriteLine($"[Hook] KeyUp vk={vk} filtered simulated");
				return;
			}
			if (vk == hsIgnoreNextReleaseFor)
			{
		...
		private void RecomputeHotstringArming()
		{
			...
			if (ready != null)
			{
				...
				var caseMode = CaseConformModes.None;
				if (ready.conformToCase)
				{
					var span = System.Runtime.InteropServices.CollectionsMarshal.AsSpan(hm.hsBuf);
					int end = span.Length - (ready.endCharRequired ? 1 : 0);
					int cap = 0, up = 0;
					for (int i = 0; i < end; i++)
					{
						char c = span[i];
		...
		private void TryPostHotkey(uint vk, bool keyUp)
		{
		...
		private void PostHotkey(uint hotkeyId, uint sc /* typically 0 on Linux */, ulong extraInfo /* 0 here */)
		{
			var inputLevel = KeyboardMouseSender.InputLevelFromInfo((long)extraInfo);
			// Mirror Windows: pack SC (low word) + input level (high word), variant omitted.
			_ = channel.Writer.TryWrite(new KeysharpMsg
			{
				message = (uint)UserMessages.AHK_HOOK_HOTKEY,
				wParam = new nint(hotkeyId),
				lParam = new nint(KeyboardUtils.MakeLong((short)sc, (short)inputLevel)),
