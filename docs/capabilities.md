...
| Image | 🟢 Full | 🟡 Partial | ⚪ Unknown | ⚪ Unknown | Cross-platform image class: create a blank ARGB canvas (Image.Create(w, h [,background])), capture (desktop/monitor/rect/window), load (file/bitmap), draw shapes and text (Clear, DrawLine, DrawRect, FillRect, DrawEllipse, FillEllipse, DrawText, DrawImage), lazy transforms (scale/rotate/flip/crop), output (save/bitmap), pixel access and search. Replaces the earlier ImageCapture() function. |
